...
import { MapPin, Star, Plus, AlertCircle, RefreshCw, Building2, ShoppingBag, Coffee, BedDouble, Landmark, Utensils } from 'lucide-react';
            ...
            return (
              <div key={dest.id} className="card" style={{ display: 'flex', flexDirection: 'column', padding: '0', overflow: 'hidden' }}>
                  ...
                  <div style={{ width: '100%', height: '200px', overflow: 'hidden', position: 'relative' }}>
                    <img
                      ...
                      onError={(e) => {
                        ...
                        e.target.parentElement.style.background = `linear-gradient(135deg, ${catColor}33, ${catColor}11)`;
                        e.target.parentElement.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:100%;"><${CatIcon.name} size="48" color="${catColor}" opacity="0.3"/></div>`;
                      }}
                    />
                ...
                <div style={{ padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
                  <h3 style={{ fontSize: '1.05rem', fontWeight: 700, marginBottom: '0.4rem' }}>{dest.name}</h3>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', color: 'var(--text-muted)', fontSize: '0.8rem', marginBottom: '0.6rem' }}>
